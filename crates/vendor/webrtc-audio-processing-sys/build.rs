...
/// Symbol prefix for the webrtc-audio-processing library to allow multiple
/// versions to coexist.
...
fn prefix_archive_symbols(
	...
	eprintln!("Prefixing {} symbols in {} with '{}'", symbols.len(), archive_path.display(), prefix);
...
mod webrtc {
	...
	pub(super) fn get_build_paths() -> Result<(Vec<PathBuf>, Vec<PathBuf>)> {
		...
		if include_path.is_none() || lib_path.is_none() {
			bail!(
				"Couldn't find {}. Please install it or set WEBRTC_AUDIO_PROCESSING_INCLUDE and \
				 WEBRTC_AUDIO_PROCESSING_LIB environment variables.",
				LIB_NAME
			);
		}
	...
	pub(super) fn prefix_library_symbols(
		...
		println!(
			"cargo:warning=Symbol prefixing is only supported with the 'bundled' feature. Without \
			 it, linking multiple versions of this crate may cause symbol conflicts."
		);
...
mod webrtc {
	...
	pub(super) fn get_build_paths() -> Result<(Vec<PathBuf>, Vec<PathBuf>)> {
		...
		// TODO(strohel): instead of hardcoding the paths, we should consult the
		// pkgconfig file that the bundled webrtc-audio-processing build produces.
		...
		// Notes: c8896801 added support for 20250814, but the meson.build is still
		// expecting >=20240722 and the subproject will fetch 20240722. If the build
		// environment has 20250814 installed, it should still pick it up and build
		// successfully, though.
	...
	pub(super) fn build_if_necessary() -> Result<()> {
		...
		// Copy the sources to under out directory so that we can patch it without
		// consequences.
		let mut cp = Command::new("cp");
		// Copy recursively, preserve attributes. Use trailing dot trick to prevent
		// creating `webrtc-audio-processing/webrtc-audio-processing` nesting on a 2nd
		// invocation.
		cp.arg("-a")
			.arg(bundled_source_path.join("."))
	...
	pub(super) fn prefix_library_symbols(lib_dirs: &[PathBuf], prefix: &str) -> Result<Vec<String>> {
	...
	///
	/// Patched for oh-my-pi: Apple's `nm` rejects the GNU long options this
	/// crate hardcodes (`--defined-only --format=posix`) and uses `-U -P`
	/// instead. Its POSIX output also interleaves `archive.a[member.o]:`
	/// header lines, which must not be mistaken for symbol names.
	fn get_defined_symbols(archive_path: &std::path::Path) -> Result<Vec<String>> {
		let apple = cfg!(target_vendor = "apple");
		let args: &[&str] = if apple {
			&["-U", "-P"]
		} else {
		...
		for line in stdout.lines() {
			// Archive and member headers end in ':' and carry no symbol.
			let line = line.trim();
			if line.is_empty() || line.ends_with(':') {
				continue;
			}
			// POSIX format: "symbol_name type value size". A symbol row always
			// has a single-character type field after the name.
...
impl ParseCallbacks for CustomDeriveCallbacks {
	fn add_derives(&self, info: &DeriveInfo) -> Vec<String> {
		...
		// Matches AudioProcessing_Config, AudioProcessing_Config_EchoCanceller
		// etc
		} else if info.name.starts_with("AudioProcessing_Config") {
			// Only derive Default for AudioProcessing_Config and its inner structs. bindgen
			// Default implementation ignores C/C++ struct default values and thus
			// misleading to enable globally. Note that we don't expose these defaults on
			// `webrtc-audio-processing` level: they are needed only by the code that
			// converts from prettified Rust config structs into their FFI variants to
			// construct disabled/dummy values.
			vec!["Default".into()]
		} else {
			vec![]
		}
	...
	fn add_attributes(&self, info: &AttributeInfo<'_>) -> Vec<String> {
		if info.name.starts_with("EchoCanceller3Config") {
			...
			// The only allowed API is through the wrapper struct in the
			// webrtc_audio_processing crate.
...
fn main() -> Result<()> {
	...
	// "cargo:rustc-link-lib=static=webrtc_audio_processing_wrapper". The wrapper
	// library should be linked before webrtc-audio-processing-2, otherwise strict
	// linkers (like when passing -Wl,--as-needed) may discard the c++ library
	// (automatically added by cc) from the linking list, resulting in build
	// failure. The linking order should respect the dependency graph, i.e. wrapper
	// -> webrtc-2.
	cc_build
		.cpp(true)
	...
	// Inform wrapper code that headers for internal classes (ResidualEchoDetector)
	// are available.
	#[cfg(feature = "bundled")]
	...
	// The the cc and bindgen commands emit `cargo:rerun-if-env-changed=...`, and
	// these deactivate the default behavior to rerun if _any_ source file changes.
	// So state these explicitly. build.rs is always included and doesn't have to
	// be specified.
	println!("cargo:rerun-if-changed=src/wrapper.hpp");
	println!("cargo:rerun-if-changed=src/wrapper.cpp");

	// Prefix the wrapper library's references to webrtc symbols to match the
	// renamed webrtc library.
...
/// Reliably determine a path to objcopy binary bundled with the active Rust
/// toolchain (rust-objcopy)
fn determine_objcopy_path() -> Result<PathBuf> {
	...
	if !objcopy.exists() {
		...
		println!(
			"cargo:warning=Ensure the 'llvm-tools' component is installed: 'rustup component add \
			 llvm-tools'"
		);
	}
