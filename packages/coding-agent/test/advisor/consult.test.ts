...
describe("consult_advisor contracts", () => {
	...
	describe("AdvisorRuntime.consult", () => {
		...
		it("does not run a ghost consult after a reset during the idle wait", async () => {
			...
			// stays parked; releasing the review must NOT let the consult prompt.
			const promptInputs: string[] = [];
			const { promise: reviewStarted, resolve: startReview } = Promise.withResolvers<void>();
			const { promise: finishReview, resolve: releaseReview } = Promise.withResolvers<void>();
			const { promise: reviewFinished, resolve: markReviewFinished } = Promise.withResolvers<void>();
			let promptCalls = 0;
			const agent: AdvisorAgent = {
				prompt: async input => {
					promptInputs.push(input);
					...
					agent.state.messages.push({
						role: "assistant",
						content: [{ type: "text", text: "ghost reply" }],
						timestamp: Date.now(),
					} as AgentMessage);
				},
				abort: () => {},
			...
			const consultPromise = runtime.consult({ message: "should never run" }).then(
				() => "resolved" as const,
				() => "rejected" as const,
			);
			await Promise.resolve();
			...
			// Reject the queued consult while it is parked on the idle waiter.
			runtime.reset();
			releaseReview();
			const settled = await consultPromise;
			// Wait for the review prompt to complete, then give the drain's finally
			// and the parked runConsult wake their microtasks — a ghost consult would
			// start within these turns if the epoch guard were missing.
			await reviewFinished;
			await flushMicrotasksUntil(() => promptCalls > 1, 10);

			expect(settled).toBe("rejected");
			expect(promptInputs).toHaveLength(1); // no ghost consult prompt
		});
