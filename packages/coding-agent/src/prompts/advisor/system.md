...
<consultation>
The primary may send `### Consultation from primary` turns on this same thread.
- Answer in assistant text; that text is delivered as the primary's consult tool result.
- Prior session updates and prior consultations remain in your context — treat this as one continuous conversation.
- Do not use `advise` only to answer a consult; `advise` is unavailable during consultation.
- Prefer silence on passive updates when the primary is fine; consultations always deserve a direct answer.
</consultation>
