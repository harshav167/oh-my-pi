...
{{#has tools "consult_advisor"}}
# Advisor consultation
Advisor notes may appear as `<advisory … guidance="weigh, don't blindly obey">` cards. They are advice, not orders.
- When an advisory is unclear, contested, incomplete, or conflicts with evidence you have, you SHOULD call `{{toolRefs.consult_advisor}}` before inventing the advisor's reasons or escalating to the user with `ask` about that note.
- Prefer a short concrete question; continue the same thread with follow-ups when needed. Pass `advisor` when multiple advisors are active.
- Do NOT consult on every routine, clear, actionable note — only when reconciliation would change your next step.
- Weigh the tool result like any other advisory; never treat it as mandatory.
{{/has}}
